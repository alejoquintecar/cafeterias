...
class ProductosVenderController extends BaseController{
  ...
  */
  ...
  public function index(){
    ...
    $this->render('productosVender/index.html.php', array(
  ...
  public function indexJson( $bExportar = true ){
    ...
    if( $_SERVER['REQUEST_SCHEME'] == 'http' || $bExportar == true ){
      ...
      // DQL Data
      $oQueryProductos = $this->oConexionDB->executeQuery("SELECT pd.id, pd.nombre, pd.peso, pd.categoria,
        pd.stock, vt.cantidad as vtCantidad, vt.precio as vtPrecio
        FROM ventas as vt
        JOIN productos as pd ON vt.producto_id = pd.id
        WHERE pd.estado = 1
      ");
      ...
      if( $oQueryProductos->num_rows > 0 ){
        while( $row = $oQueryProductos->fetch_object() ){
          $aProductos[] = array(
            ...
            'vtCantidad' => $row->vtCantidad,
            'vtPrecio'   => $row->vtPrecio,
            'stock'      => $row->stock,
            'peso'       => $row->peso,
          );
        }
